...
import { neonWorkflow } from "./workflow.ts";
...
export const usage =
  ...
  "  create    provision and verify the Neon serverless Postgres server\n" +
...
export async function run(...args: string[]): Promise<Opts> {
  ...
  if (lifecycleCommands.includes(command)) {
    return runCli(neonWorkflow, withFile);
  }
...
export async function exec(args: string[] = Bun.argv.slice(2)): Promise<never> {
  if (lifecycleCommands.includes(args[0] ?? "")) {
    return execCli(neonWorkflow, defaultArgs(args));
  }
