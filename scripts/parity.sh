...
build_variant() {
  ...
  (cd "$root/green" && NEON_LIB_ROOT="$root" ./green build -f "$tmp/$variant-green.yml" >/dev/null)
  (cd "$root/red" && NEON_LIB_ROOT="$root/red" ./red build -f "$tmp/$variant-red.yml" >/dev/null)
  (cd "$root/blue" && uv run python -m package_neon_blue build -f "$tmp/$variant-blue.yml" >/dev/null)
...
diff -r "$root/green/src/resources/io/github/getcolors/neon" "$root/red/resources"
diff -r "$root/green/src/resources/io/github/getcolors/neon" "$root/blue/src/package_neon_blue/resources"

echo "green, red, and blue Neon artifacts are byte-identical"
