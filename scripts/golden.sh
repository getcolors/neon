...
for variant in colors optout; do
  ...
  (cd "$root/green" && NEON_LIB_ROOT="$root" ./green build -f "$fixture" >/dev/null)
  ...
  for par in NEON_R2_ACCESS_KEY_ID NEON_R2_SECRET_ACCESS_KEY; do
    if ! grep -q "lookup('env','COLORS_PAR_$par')" "$actual/neon-ansible/main.yml"; then
  ...
  if grep -rEq '([0-9]{1,3}\.){3}[0-9]{1,3}' "$actual/neon-ansible-local"; then
