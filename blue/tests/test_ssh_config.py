...
from package_neon_blue import ssh_config, tools, workflow

# §2 the alias and the identity file


def test_alias_is_the_profile():
    assert ssh_config.host_alias(fixture()) == "neon-fixture"
...
def test_identity_file_keeps_the_tilde(tmp_path, monkeypatch):
    ...
    assert ssh_config.identity_file(fixture()) == "~/.ssh/neon-fixture"
...
def test_the_marker_is_the_alias_alone():
    ...
    # too would repeat it: "# BEGIN neon neon-vultr".
    assert ssh_config.begin_marker("neon-vultr") == "# BEGIN neon-vultr ANSIBLE MANAGED BLOCK"
    assert ssh_config.end_marker("neon-vultr") == "# END neon-vultr ANSIBLE MANAGED BLOCK"
...
def test_a_foreign_stanza_is_found():
    lines = ["Host other", "    HostName 192.0.2.1", "", "Host neon-fixture"]
    assert ssh_config.foreign_stanza_line(lines, "neon-fixture") == 4
...
def test_a_block_under_a_retired_marker_is_foreign():
    ...
    alias = "neon-vultr"
    lines = [f"# BEGIN neon {alias} ANSIBLE MANAGED BLOCK",
             f"Host {alias}",
             f"# END neon {alias} ANSIBLE MANAGED BLOCK"]
    assert ssh_config.foreign_stanza_line(lines, alias) == 2


def test_a_multi_pattern_host_line_counts():
    assert ssh_config.foreign_stanza_line(["Host web neon-fixture db"], "neon-fixture") == 1


def test_an_unrelated_file_is_left_alone():
    assert ssh_config.foreign_stanza_line(["Host build", "Host neon-other"],
                                          "neon-fixture") is None
...
def test_the_local_play_renders_no_address():
    ...
    assert data["ssh-config-identity-file"] == "~/.ssh/neon-fixture"
...
def test_the_local_stage_renders_three_files():
    ...
    assert all("neon-ansible-local" in t for t in targets)
...
def test_create_writes_the_block_after_compute_and_before_convergence():
    ...
    assert workflow.wire_fn("neon/infrastructure", create)[1:] == ("neon/ssh-config",)
    assert workflow.wire_fn("neon/ssh-config", create)[1:] == ("neon/ansible",)
