...
from package_neon_blue import workflow
...
async def test_real_create_requires_credentials():
    ...
    assert "CLOUDFLARE" not in result["blue/err"]
...
def test_graph_orders_the_stack():
    ...
    assert workflow.wire_fn("neon/start", create)[1:] == ("neon/infrastructure",)
    ...
    assert workflow.wire_fn("neon/ansible", create)[1:] == ("neon/acceptance",)
...
def test_backend_addresses_key_state_by_profile_and_tool():
    ...
    assert dir.endswith(".colors/neon-fixture/neon-infrastructure")
