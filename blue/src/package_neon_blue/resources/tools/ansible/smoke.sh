...
if [ "$(rclone cat "r2:$bucket/$prefix/.colors-ready" 2>/dev/null)" != "<{ profile }>" ]; then
  ready=$(mktemp); printf '%s' "<{ profile }>" > "$ready"
  ...
  if [ "$back" != "<{ profile }>" ]; then
    echo "neon-smoke: ready marker read back as '$back', expected '<{ profile }>'" >&2
    exit 1
  fi
