...
def requirements(opts):
    ssh = source_cidrs(opts, 'ssh-sources', 'neon-ssh-sources')
    return {'single_host': True, 'security': {'egress':'all', 'private_filter':False, 'ingress':[
        ...
        'legacy_state_keys':[opts['profile']+'/neon-infrastructure.tfstate']}
...
def attach(opts, result):
    ...
    if result.get('status') == 'destroyed':
        return {**opts, 'blue/exit': 0, 'neon/already-destroyed': True}
