(ns io.github.getcolors.neon.ssh-config-test
...
(deftest alias-is-the-profile
  (is (= "neon-fixture" (ssh-config/host-alias (fixture)))))
...
(deftest identity-file-keeps-the-tilde
  ...
  (is (= "~/.ssh/neon-fixture" (ssh-config/identity-file (fixture))))
...
(deftest the-marker-is-the-alias-alone
  ...
  (is (= "# BEGIN neon-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/begin-marker "neon-vultr")))
  (is (= "# END neon-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/end-marker "neon-vultr"))))

;; §5 never adopt

(deftest a-foreign-stanza-is-found
  (let [lines ["Host other" "    HostName 192.0.2.1" "" "Host neon-fixture"]]
    (is (= 4 (ssh-config/foreign-stanza-line lines "neon-fixture")))))

(deftest our-own-block-is-not-foreign
  (let [alias "neon-fixture"
...
(deftest a-block-under-a-retired-marker-is-foreign
  ;; The superseded `# BEGIN neon <alias>` marker is gone, so a block
  ...
  (let [alias "neon-vultr"
        lines [(str "# BEGIN neon " alias " ANSIBLE MANAGED BLOCK")
               (str "Host " alias)
               (str "# END neon " alias " ANSIBLE MANAGED BLOCK")]]
    (is (= 2 (ssh-config/foreign-stanza-line lines alias)))))

(deftest a-multi-pattern-host-line-counts
  (is (= 1 (ssh-config/foreign-stanza-line ["Host web neon-fixture db"]
                                           "neon-fixture"))))

(deftest an-unrelated-file-is-left-alone
  (is (nil? (ssh-config/foreign-stanza-line ["Host build" "Host neon-other"]
                                            "neon-fixture"))))
...
(deftest the-local-play-renders-no-address
  ...
  (let [data (tools/ansible-local-data (assoc (fixture) :ip "203.0.113.7"))]
    ...
    (is (= "~/.ssh/neon-fixture" (:ssh-config-identity-file data)))))
...
(deftest the-local-stage-renders-three-files
  (let [targets (map #(str (:target %)) (tools/ansible-local-specs (fixture)))]
    ...
    (is (every? #(str/includes? % "neon-ansible-local") targets))))
