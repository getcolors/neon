(ns io.github.getcolors.neon.compute
...
(defn requirements [opts]
 {:single_host true :security {:egress "all" :private_filter false :ingress [{:id "ssh" :protocol "tcp" :from_port 22 :to_port 22 :sources (deployment/source-cidrs opts "ssh-sources" "neon-ssh-sources")}]}
  :legacy_state_keys [(str (:profile opts) "/neon-infrastructure.tfstate")]})
...
(defn attach [opts result]
 (cond
  ...
  (= "destroyed" (:status result)) (assoc opts :green/exit 0 :neon/already-destroyed true)
